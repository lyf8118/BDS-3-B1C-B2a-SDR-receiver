...
if (fid > 0)
   ...
   [data, count] = fread(fid, [1, dataAdaptCoeff*5*samplesPerCode], settings.dataType);


    fclose(fid);

    if (count < dataAdaptCoeff*5*samplesPerCode)
    ...
    timeScale = 0 : 1/settings.samplingFreq : 5e-2;
