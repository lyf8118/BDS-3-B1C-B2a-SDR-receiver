...
if (fid > 0)
   ...
   [data, count] = fread(fid, [1, dataAdaptCoeff*10*samplesPerCode], settings.dataType);


    fclose(fid);

    if (count < dataAdaptCoeff*10*samplesPerCode)
    ...
    timeScale = 0 : 1/settings.samplingFreq : 25e-3;

    %--- Time domain plot -------------------------------------------------
    if (settings.fileType==1)

        subplot(2, 2, 3);
        plot(1000 * timeScale(1:round(samplesPerCode/2)), ...
            data(1:round(samplesPerCode/2)));
        ...
        subplot(3, 2, 4);
        plot(1000 * timeScale(1:round(samplesPerCode/2)), ...
            real(data(1:round(samplesPerCode/2))));
        ...
        subplot(3, 2, 3);
        plot(1000 * timeScale(1:round(samplesPerCode/2)), ...
            imag(data(1:round(samplesPerCode/2))));
