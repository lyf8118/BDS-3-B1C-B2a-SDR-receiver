function acqResults = GPU_acquisition(longSignal, settings)
%Function performs cold start acquisition on the collected "data", using 
%GPU acceleration. It searches for GPS signals of all satellites, which 
%are listed in field "acqSatelliteList" in the settings structure. 
%Function saves code phase and frequency of the detected signals in the
%"acqResults" structure.
%
...
sig10PlusXms = gpuArray(single(sig10PlusXms));
...
phasePoints = gpuArray(single(phasePoints));
...
results     = single(zeros(numberOfFrqBins, len10PlusXms));
...
%% Correlate signals ======================================================   
    results     = gpuArray(results);
    % Generate all B1C codes and sample them according to the sampling freq.
    ...
    localData = gpuArray(single(localData));
    ...
    if (settings.pilotACQflag == 1)
        ...
        localPilot = [PilotPriTable(1:samplesXmsLen) ...
                                  zeros(1,len10PlusXms - samplesXmsLen)];
        localPilot = gpuArray(single(localPilot));
    ...
    for frqBinIndex = 1:numberOfFrqBins
        ...
        sigCarr = exp(1i*single(frqBins(frqBinIndex)) * phasePoints);
...
%% Look for correlation peaks in the results ==============================
    ...
    acqResults.peakMetric(PRN) = peakSize/gather(sigPower);
